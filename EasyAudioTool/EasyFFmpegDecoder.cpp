...
qint64 EasyFFmpegDecoder::read(char *outBuffer, qint64 maxSize)
{
    if(!isOpen() || maxSize < 1)
        return -1;
    if(atEnd())
        ...
        if(packet->stream_index == theContextPtr->audioStreamIndex)
        {
            ...
            while (avcodec_receive_frame(theContextPtr->codecCtx, frame) == 0)
            {
                ...
                if(out_bufuse > 0){
                    ...
                    dataTemp.append((const char*)out_buffer, out_bufuse);
                }
                av_frame_unref(frame);
...
qint64 EasyFFmpegDecoder::readAll(std::function<bool (const char *, qint64)> callBack)
{
    if(!isOpen() || !callBack)
        return -1;
    //先重置读取位置
