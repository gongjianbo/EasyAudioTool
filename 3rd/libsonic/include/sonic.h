...
DLLAPI sonicStream sonicCreateStream(int sampleRate, int numChannels);
/* Destroy the sonic stream. */
DLLAPI void sonicDestroyStream(sonicStream stream);
/* Use this to write floating point data to be speed up or down into the stream.
   ...
   otherwise 1 */
DLLAPI int sonicWriteFloatToStream(sonicStream stream, float* samples, int numSamples);
/* Use this to write 16-bit data to be speed up or down into the stream.
   Return 0 if memory realloc failed, otherwise 1 */
DLLAPI int sonicWriteShortToStream(sonicStream stream, short* samples, int numSamples);
/* Use this to write 8-bit unsigned data to be speed up or down into the stream.
   Return 0 if memory realloc failed, otherwise 1 */
DLLAPI int sonicWriteUnsignedCharToStream(sonicStream stream, unsigned char* samples,
                                   int numSamples);
...
DLLAPI int sonicReadFloatFromStream(sonicStream stream, float* samples,
                             int maxSamples);
/* Use this to read 16-bit data out of the stream.  Sometimes no data will
   be available, and zero is returned, which is not an error condition. */
DLLAPI int sonicReadShortFromStream(sonicStream stream, short* samples,
                             int maxSamples);
/* Use this to read 8-bit unsigned data out of the stream.  Sometimes no data
   will be available, and zero is returned, which is not an error condition. */
DLLAPI int sonicReadUnsignedCharFromStream(sonicStream stream, unsigned char* samples,
                                    int maxSamples);
...
DLLAPI int sonicFlushStream(sonicStream stream);
/* Return the number of samples in the output buffer */
DLLAPI int sonicSamplesAvailable(sonicStream stream);
/* Get the speed of the stream. */
DLLAPI float sonicGetSpeed(sonicStream stream);
/* Set the speed of the stream. */
DLLAPI void sonicSetSpeed(sonicStream stream, float speed);
/* Get the pitch of the stream. */
DLLAPI float sonicGetPitch(sonicStream stream);
/* Set the pitch of the stream. */
DLLAPI void sonicSetPitch(sonicStream stream, float pitch);
/* Get the rate of the stream. */
DLLAPI float sonicGetRate(sonicStream stream);
/* Set the rate of the stream. */
DLLAPI void sonicSetRate(sonicStream stream, float rate);
/* Get the scaling factor of the stream. */
DLLAPI float sonicGetVolume(sonicStream stream);
/* Set the scaling factor of the stream. */
DLLAPI void sonicSetVolume(sonicStream stream, float volume);
/* Get the chord pitch setting. */
DLLAPI int sonicGetChordPitch(sonicStream stream);
/* Set chord pitch mode on or off.  Default is off.  See the documentation
   page for a description of this feature. */
DLLAPI void sonicSetChordPitch(sonicStream stream, int useChordPitch);
/* Get the quality setting. */
DLLAPI int sonicGetQuality(sonicStream stream);
/* Set the "quality".  Default 0 is virtually as good as 1, but very much
 * faster. */
DLLAPI void sonicSetQuality(sonicStream stream, int quality);
/* Get the sample rate of the stream. */
DLLAPI int sonicGetSampleRate(sonicStream stream);
/* Set the sample rate of the stream.  This will drop any samples that have not
 * been read. */
DLLAPI void sonicSetSampleRate(sonicStream stream, int sampleRate);
/* Get the number of channels. */
DLLAPI int sonicGetNumChannels(sonicStream stream);
/* Set the number of channels.  This will drop any samples that have not been
 * read. */
DLLAPI void sonicSetNumChannels(sonicStream stream, int numChannels);
/* This is a non-stream oriented interface to just change the speed of a sound
   ...
   samples. */
DLLAPI int sonicChangeFloatSpeed(float* samples, int numSamples, float speed,
...
/* This is a non-stream oriented interface to just change the speed of a sound
   ...
   speed*numSamples available space in the array. Returns the new number of
   samples. */
DLLAPI int sonicChangeShortSpeed(short* samples, int numSamples, float speed,
                          float pitch, float rate, float volume,
                          int useChordPitch, int sampleRate, int numChannels);
