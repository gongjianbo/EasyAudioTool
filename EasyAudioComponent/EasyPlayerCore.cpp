...
void EasyPlayerCore::play(const QString &filepath)
{
    ...
    //根据路径构建解码对象
    QSharedPointer<EasyAbstractDecoder> audio_decoder =
            EasyAudioFactory::createDecoder(filepath);
    if(audio_decoder &&
            audio_decoder->isValid() &&
            audio_decoder->open(audioFormat)){
...
void EasyPlayerCore::doPlay()
{
    ...
    if(!audioOutput){
        ...
        connect(audioOutput,&QAudioOutput::notify,this,[this]{
            ...
            setPosition(play_ms);
            //qDebug()<<play_ms<<audioBuffer.getReadCount()<<audioBuffer.getWriteCount()<<audioBuffer.isWriteEnd();
            if(audioBuffer.isWaitWrite() && audioDecoder && !audioDecoder->isEnd()){
                //每次读大约一分钟的数据
                audioBuffer.appendData(audioDecoder->read(EasyPlayerBuffer::getOnceLength()));
                if(audioDecoder->isEnd()){
